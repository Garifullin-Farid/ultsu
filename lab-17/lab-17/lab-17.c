...
void insertElem(int ins,int value) {
	for (int i = n; i > ins; i--)
	{
		arr[i] = arr[i - 1];
	}
...
void main() {
	...
	do
	{
		...
		printf("14: 0 перед минимальным\n");
		printf("15: сохранить состояние масива в текстовом файле\n");
		printf("16: ввести состояние документа из текстового файла\n");
		...
		{
			int i, value;
			printf("Перед каким элементом нужно вставить индекс? Индекс =");
			scanf_s("%d", &i);
			printf("значение которок нужно вставит = ");
			scanf_s("%d", &value);
			insertElem(i, value);
			break;
		}
		...
		case 15: {
			FILE* fout = fopen("D:\\уник\\ОАиП_(Савельев)\\files\\out7.txt", "wt");
			if (fout == NULL) {
			...
			printf("состояние масива сохранено");
			fclose(fout);
			break;
		}
		case 16: {
			FILE* fin = fopen("D:\\уник\\ОАиП_(Савельев)\\files\\in7.txt", "rt");
			if (fin == NULL) {
